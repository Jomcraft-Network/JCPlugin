...
public class FileUtilNoMC {
    ...
    public static PrivateJSON getPrivateJSON() {
        ...
        if (privateJson != null) return privateJson;
    ...
    public static MainJSON getMainJSON() {
        ...
        if (mainJson != null) return mainJson;
    ...
    public static void restoreContentsFirst() throws NullPointerException, IOException, NoSuchAlgorithmException {
        ...
        if (!new File(getMainFolder(), mainJson.mainProfile).exists()) mainJson.mainProfile = firstFolder;

        if (privateJson.targetProfile.equals("!NEW!")) privateJson.targetProfile = mainJson.mainProfile;

        if (privateJson.currentProfile.equals("!NEW!")) privateJson.currentProfile = mainJson.mainProfile;

        if (!new File(getMainFolder(), privateJson.targetProfile).exists()) privateJson.targetProfile = firstFolder;

        if (!new File(getMainFolder(), privateJson.currentProfile).exists()) privateJson.currentProfile = firstFolder;
    ...
    private static boolean switchProfile() throws IOException {
        if (mainJson.generatedBy.equals("<default>")) mainJson.generatedBy = privateJson.privateIdentifier;
    ...
    private static void initialToDefaultProfile() {
        if (getMainJSON().mainProfile.equals("!NEW!")) {
            ...
            FileFilter ffm = new FileFilter() {
                ...
                @Override
                public boolean accept(File file) {
                    if (!file.getName().equals("Default")) return true;
            ...
            try {
                ...
                if (opFile.exists()) {
                    ...
                    while ((lineOptions = readerOptions.readLine()) != null) {
                        if (lineOptions.startsWith("key_")) list.add(lineOptions);
                    }
                }
                ...
                String line;
                while ((line = reader.readLine()) != null) writer.print(line + "\n");
                ...
                try {
                    reader.close();
                    writer.close();
                    if (existed) readerOptions.close();
                } catch (IOException e) {
                    throw e;
    ...
    public static void restoreServers() throws IOException {
        try {
            ...
            if (file.exists()) FileUtils.copyFile(file, new File(mcDataDir, "servers.dat"));
            else JCLogger.log.log(Level.WARN, "Couldn't restore the server config as it's not included");
        } catch (IOException e) {
        ...
        try {
            ...
            for (File configFile : config) {
                if (!configFile.isDirectory() && !configFile.getName().equals("ignore.json") && !configFile.getName().contains("defaultsettings") && !optUse.contains(configFile.getName())) {
                    ...
                    if (!mainJson.hashes.containsKey(pathString)) continue;
    ...
    public static ArrayList<String> listConfigFiles() throws FileNotFoundException, IOException {
        ...
        for (File configFile : new File(getMainFolder(), activeProfile).listFiles()) {
            if (!configFile.getName().equals("ignore.json") && !configFile.getName().contains("defaultsettings")) {
                if (optUse.contains(configFile.getName())) continue;
                String relativePath = configFile.getPath().substring((mcDataDir.getPath().length()));
